...
func TestBoundTokenSignerController(t *testing.T) {
	...
	operatorSecret, err := kubeClient.Secrets(operatorNamespace).Get(tokenctl.NextPrivateKeySecretName, metav1.GetOptions{})
	require.NoError(t, err)
	...
	t.Run("private-key-deletion", func(t *testing.T) {
		err := kubeClient.Secrets(targetNamespace).Delete(tokenctl.PrivateKeySecretName, &metav1.DeleteOptions{})
		require.NoError(t, err)
		checkBoundTokenOperandSecret(t, kubeClient, operatorSecret.Data)
	})

	// The operand config map should be recreated after deletion. This will
	// also serve to reset the contents of the config map to only include the
	// current public key so that the containing test would pass if run again.
	t.Run("configmap-deletion", func(t *testing.T) {
		err := kubeClient.ConfigMaps(targetNamespace).Delete(tokenctl.CertConfigMapName, &metav1.DeleteOptions{})
		require.NoError(t, err)
		checkCertConfigMap(t, kubeClient, map[string]string{
			"bound-service-account-001.pub": string(operatorPublicKey),
		})
	})
	...
	// immediately, and the secret once the ready interval has passed.
	t.Run("next-private-key-deletion", func(t *testing.T) {
		// Delete the operator secret
		err := kubeClient.Secrets(operatorNamespace).Delete(tokenctl.NextPrivateKeySecretName, &metav1.DeleteOptions{})
		require.NoError(t, err)
		...
		err = wait.PollImmediate(interval, timeout, func() (done bool, err error) {
			newOperatorSecret, err = kubeClient.Secrets(operatorNamespace).Get(tokenctl.NextPrivateKeySecretName, metav1.GetOptions{})
		...
		// Annotation should have a time set in the future
		readyTime := newOperatorSecret.Annotations[tokenctl.TokenReadyAnnotation]
		require.Greater(t, len(readyTime), 0)
		promotionTime, err := time.Parse(time.RFC3339, readyTime)
		require.NoError(t, err)
		require.True(t, time.Now().Before(promotionTime))
		...
		checkCertConfigMap(t, kubeClient, map[string]string{
			"bound-service-account-001.pub": string(operatorPublicKey),
			"bound-service-account-002.pub": string(newOperatorPublicKey),
		})

		// The operand secret should remain unchanged due to the ready annotation
		checkBoundTokenOperandSecret(t, kubeClient, operatorSecret.Data)

		// Clearing the ready annotation should result in update of the operand secret
		delete(newOperatorSecret.Annotations, tokenctl.TokenReadyAnnotation)
		_, err = kubeClient.Secrets(operatorNamespace).Update(newOperatorSecret)
		require.NoError(t, err)
		checkBoundTokenOperandSecret(t, kubeClient, newOperatorSecret.Data)
	})
}

// checkBoundTokenOperandSecret checks that the operand secret and populated with
// the expected data.
func checkBoundTokenOperandSecret(t *testing.T, kubeClient *clientcorev1.CoreV1Client, expectedData map[string][]byte) {
	err := wait.PollImmediate(interval, timeout, func() (done bool, err error) {
		secret, err := kubeClient.Secrets(operatorclient.TargetNamespace).Get(tokenctl.PrivateKeySecretName, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		if err != nil {
...
func checkCertConfigMap(t *testing.T, kubeClient *clientcorev1.CoreV1Client, expectedData map[string]string) {
	err := wait.PollImmediate(interval, timeout, func() (done bool, err error) {
		configMap, err := kubeClient.ConfigMaps(operatorclient.TargetNamespace).Get(tokenctl.CertConfigMapName, metav1.GetOptions{})
		if apierrors.IsNotFound(err) {
			return false, nil
		}
		if err != nil {
...
// TestTokenRequestAndReview checks bound tokens are configured correct. A token is
// requested via the TokenRequest API and validated via the TokenReview API.
func TestTokenRequestAndReview(t *testing.T) {
	t.Skip("TokenRequest not yet configured")
	...
	// TODO(marun) Validate a projected token (may involve pod security)
	// TODO(marun) Validate the impact of a change in issuer on a projected token
}

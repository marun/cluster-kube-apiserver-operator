...
func ReadStaticPodManifests(manifestDir, podNamePrefix, containerName string) (StaticPodManifests, error) {
	...
	for _, file := range files {
		// Look only for manifests of the form prefix-revision.yaml
		// TODO(marun) Improve handling of non-graceful suffix (should
		// not co-exist with revisioned suffixes).
		if !strings.HasPrefix(file.Name(), podNamePrefix) || strings.HasSuffix(file.Name(), "-pod.yaml") {
			continue
		}
